...
class CreateProjectForm(forms.Form):
    project_name = forms.CharField(label='Projektin nimi', required=False, help_text="Anna projektin nimi", max_length=200,widget=forms.TextInput(attrs={'autocomplete':'off'}))
    destination_name = forms.CharField(label='Kohteen nimi', required=False, help_text="Anna kohteen nimi", max_length=200,widget=forms.TextInput(attrs={'autocomplete':'off'}))
    start_date = forms.DateTimeField(label='Aloituspäivä', required=False, help_text="Projektin aloittamispäivämäärä", input_formats=[constants.DATE_FORMAT], widget=FengyuanChenDatePickerInput(attrs={'autocomplete':'off'}))
    end_date = forms.DateTimeField(label='Lopetuspäivä', required=False, help_text="Projektin lopettamispäivämäärä", input_formats=[constants.DATE_FORMAT], widget=FengyuanChenDatePickerInput(attrs={'autocomplete':'off'}))
    structure_type = forms.CharField(label='Rakennustyyppi', required=False, help_text="Projektin kohteen rakennustyyppi", widget=forms.Select(choices=constants.STRUCTURE_TYPES))
    building_material = forms.CharField(label='Rakennusmateriaali', required=False, help_text="Projektin kohteen rakennusmateriaali", widget=forms.Select(choices=constants.BUILDING_MATERIALS))
    service = forms.CharField(label='Palvelu', required=False, help_text="Projektissa tarjoamamme palvelu", widget=forms.Select(choices=constants.SERVICES))
    construction_operation = forms.CharField(label='Rakennustoimenpide', required=False, help_text="Projektin rakennustoimenpide", widget=forms.Select(choices=constants.CONSTRUCTION_OPERATIONS))
