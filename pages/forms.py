...
class SearchProjectForm(forms.Form):
    start_date = forms.DateField(required=False,label='Aloitus pvm',widget=forms.SelectDateWidget(empty_label='—',years=constants.YEARS[1:]))
    end_date = forms.DateField(required=False,label='Lopetus pvm',widget=forms.SelectDateWidget(empty_label='—',years=constants.YEARS[1:]))
...
class CreateProject(forms.Form):
    ...
    start_date = forms.DateTimeField(label='Aloituspäivä', input_formats=['%Y-%d-%m'], widget=FengyuanChenDatePickerInput(attrs={'autocomplete':'off'}))
    end_date = forms.DateTimeField(label='Lopetuspäivä', input_formats=['%Y-%d-%m'], widget=FengyuanChenDatePickerInput(attrs={'autocomplete':'off'}))
    ...
    specific_project_type = forms.CharField(required=False,empty_value=None,label='Osaamisalue',max_length=200)
    project_manager = forms.CharField(required=False,empty_value=None,label='Projektin vetäjä',max_length=100)
    ...
    documentation_path = forms.CharField(required=False,empty_value=None,label='Polku tiedostojen sijaintiin',max_length=200)